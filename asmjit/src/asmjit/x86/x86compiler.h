...
struct X86CompilerVar : public CompilerVar
{
  ...
  inline GpVar asGpVar() const ASMJIT_NOTHROW
  {
    GpVar var;
    var._var.id = id;
    var._var.size = size;
    var._var.regCode = x86VarInfo[type].getCode();
    var._var.varType = type;
    return var;
  }

  inline MmVar asMmVar() const ASMJIT_NOTHROW
  {
    MmVar var;
    var._var.id = id;
    var._var.size = size;
  ...
  inline XmmVar asXmmVar() const ASMJIT_NOTHROW
  {
    XmmVar var;
    var._var.id = id;
    var._var.size = size;
  ...
  //! register allocator should neber reserve memory for this variable, because
...
struct X86Compiler : public Compiler
{
  ...
  ASMJIT_API X86Compiler(Context* context = JitContext::getGlobal()) ASMJIT_NOTHROW;
  //! @brief Destroy the @ref X86Compiler instance.
  ASMJIT_API ~X86Compiler() ASMJIT_NOTHROW;
  ...
  inline X86CompilerFuncDecl* newFunc(uint32_t convention, const FuncPrototype& func) ASMJIT_NOTHROW
  ...
  ASMJIT_API void _emitInstruction(uint32_t code) ASMJIT_NOTHROW;

  //! @brief Emit instruction with one operand.
  ASMJIT_API void _emitInstruction(uint32_t code, const Operand* o0) ASMJIT_NOTHROW;

  //! @brief Emit instruction with two operands.
  ASMJIT_API void _emitInstruction(uint32_t code, const Operand* o0, const Operand* o1) ASMJIT_NOTHROW;

  //! @brief Emit instruction with three operands.
  ASMJIT_API void _emitInstruction(uint32_t code, const Operand* o0, const Operand* o1, const Operand* o2) ASMJIT_NOTHROW;

  //! @brief Emit instruction with four operands (Special instructions).
  ASMJIT_API void _emitInstruction(uint32_t code, const Operand* o0, const Operand* o1, const Operand* o2, const Operand* o3) ASMJIT_NOTHROW;

  //! @brief Emit instruction with five operands (Special instructions).
  ASMJIT_API void _emitInstruction(uint32_t code, const Operand* o0, const Operand* o1, const Operand* o2, const Operand* o3, const Operand* o4) ASMJIT_NOTHROW;

  //! @brief Private method for emitting jcc.
  ASMJIT_API void _emitJcc(uint32_t code, const Label* label, uint32_t hint) ASMJIT_NOTHROW;

  //! @brief Private method for emitting function call.
  ASMJIT_API X86CompilerFuncCall* _emitCall(const Operand* o0) ASMJIT_NOTHROW;

  //! @brief Private method for returning a value from the function.
  ASMJIT_API void _emitReturn(const Operand* first, const Operand* second) ASMJIT_NOTHROW;
  ...
  ASMJIT_API void align(uint32_t m) ASMJIT_NOTHROW;
  ...
  ASMJIT_API void bind(const Label& label) ASMJIT_NOTHROW;
  ...
  inline X86CompilerVar* _getVar(uint32_t id) const ASMJIT_NOTHROW
  {
  ...
  ASMJIT_API X86CompilerVar* _newVar(const char* name, uint32_t type, uint32_t size) ASMJIT_NOTHROW;

  //! @brief Create a new general-purpose variable.
  ASMJIT_API GpVar newGpVar(uint32_t varType = kX86VarTypeGpz, const char* name = NULL) ASMJIT_NOTHROW;
  //! @brief Get argument as general-purpose variable.
  ASMJIT_API GpVar getGpArg(uint32_t argIndex) ASMJIT_NOTHROW;

  //! @brief Create a new MM variable.
  ASMJIT_API MmVar newMmVar(uint32_t varType = kX86VarTypeMm, const char* name = NULL) ASMJIT_NOTHROW;
  //! @brief Get argument as MM variable.
  ASMJIT_API MmVar getMmArg(uint32_t argIndex) ASMJIT_NOTHROW;

  //! @brief Create a new XMM variable.
  ASMJIT_API XmmVar newXmmVar(uint32_t varType = kX86VarTypeXmm, const char* name = NULL) ASMJIT_NOTHROW;
  //! @brief Get argument as XMM variable.
  ASMJIT_API XmmVar getXmmArg(uint32_t argIndex) ASMJIT_NOTHROW;

  //! @internal
  //!
  //! @brief Serialize variable hint.
  ASMJIT_API void _vhint(Var& var, uint32_t hintId, uint32_t hintValue) ASMJIT_NOTHROW;

  //! @brief Alloc variable @a var.
  ASMJIT_API void alloc(Var& var) ASMJIT_NOTHROW;
  //! @brief Alloc variable @a var using @a regIndex as a register index.
  ASMJIT_API void alloc(Var& var, uint32_t regIndex) ASMJIT_NOTHROW;
  //! @brief Alloc variable @a var using @a reg as a demanded register.
  ASMJIT_API void alloc(Var& var, const Reg& reg) ASMJIT_NOTHROW;
  //! @brief Spill variable @a var.
  ASMJIT_API void spill(Var& var) ASMJIT_NOTHROW;
  //! @brief Save variable @a var if modified.
  ASMJIT_API void save(Var& var) ASMJIT_NOTHROW;
  //! @brief Unuse variable @a var.
  ASMJIT_API void unuse(Var& var) ASMJIT_NOTHROW;
  ...
  ASMJIT_API uint32_t getPriority(Var& var) const ASMJIT_NOTHROW;
  //! @brief Set priority of variable @a var to @a priority.
  ASMJIT_API void setPriority(Var& var, uint32_t priority) ASMJIT_NOTHROW;

  //! @brief Get save-on-unuse @a var property.
  ASMJIT_API bool getSaveOnUnuse(Var& var) const ASMJIT_NOTHROW;
  //! @brief Set save-on-unuse @a var property to @a value.
  ASMJIT_API void setSaveOnUnuse(Var& var, bool value) ASMJIT_NOTHROW;

  //! @brief Rename variable @a var to @a name.
  //!
  //! @note Only new name will appear in the logger.
  ASMJIT_API void rename(Var& var, const char* name) ASMJIT_NOTHROW;
  ...
  ASMJIT_API X86CompilerState* _newState(uint32_t memVarsCount) ASMJIT_NOTHROW;
  ...
  ASMJIT_API virtual void* make() ASMJIT_NOTHROW;

  //! @brief Method that will emit everything to @c Assembler instance @a a.
  ASMJIT_API virtual void serialize(Assembler& a) ASMJIT_NOTHROW;
